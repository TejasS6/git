...
def set_tile(row, column):
    ...
    # if the game is not over and it's AI's turn, let AI make a move
    if not game_over and curr_player == playerO:
        ai_move()

def ai_move():
    available_spots = [(row, column) for row in range(3) for column in range(3) if board[row][column]["text"] == ""]
    if available_spots:
        row, column = random.choice(available_spots)
        set_tile(row, column)
...
def new_game():
    ...
    turns = 0
    game_over = False
    curr_player = playerX  # X always starts
...
label = tk.Label(frame, text=curr_player + "'s turn", font=("Consolas", 20), background=color_gray,
                 foreground="white")
